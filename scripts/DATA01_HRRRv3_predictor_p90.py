...
#HRRRv3_lead_p0 = zarr.load(save_dir_campaign+'HRRR_{:02}_v3.zarr'.format(leads[0]))[:730, ...]
#HRRRv3_lead_p1 = zarr.load(save_dir_campaign+'HRRR_{:02}_v3.zarr'.format(leads[1]))[:730, ...]
#HRRRv3_lead_p2 = zarr.load(save_dir_campaign+'HRRR_{:02}_v3.zarr'.format(leads[2]))[:730, ...]

HRRRv3_lead_p0 = da.from_zarr(save_dir_scratch+'HRRR_{:02}_v3.zarr'.format(leads[0]))[:730, ...]
HRRRv3_lead_p1 = da.from_zarr(save_dir_scratch+'HRRR_{:02}_v3.zarr'.format(leads[1]))[:730, ...]
HRRRv3_lead_p2 = da.from_zarr(save_dir_scratch+'HRRR_{:02}_v3.zarr'.format(leads[2]))[:730, ...]
...
for ix in range(shape_80km[0]):
    for iy in range(shape_80km[1]):
        ...
        if land_mask_80km[ix, iy]:
            save_name = '/glade/work/ksha/NCAR/p90_v3_80km_ix{}_iy{}_lead{}{}{}.npy'.format(ix, iy, leads[0], leads[1], leads[2])
            # ------ only for re-run ----- #
            if os.path.isfile(save_name):
                print('{} Exists'.format(save_name))
            else:
                p0 = np.array(HRRRv3_lead_p0[:, x_edge_left:x_edge_right, y_edge_bottom:y_edge_top, :])
                p1 = np.array(HRRRv3_lead_p1[:, x_edge_left:x_edge_right, y_edge_bottom:y_edge_top, :])
                p2 = np.array(HRRRv3_lead_p2[:, x_edge_left:x_edge_right, y_edge_bottom:y_edge_top, :])
