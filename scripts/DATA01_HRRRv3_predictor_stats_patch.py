...
for ix in range(shape_80km[0]):
    for iy in range(shape_80km[1]):
        ...
        if land_mask_80km[ix, iy]:
            save_name = '/glade/work/ksha/NCAR/stats_v3_80km_ix{}_iy{}_lead{}{}{}.npy'.format(ix, iy, leads[0], leads[1], leads[2])
            # ------ only for re-run ----- #
            if os.path.isfile(save_name):
                print('{} Exists'.format(save_name))
            else:      
                # start_time = time.time()
                p0 = HRRRv3_lead_p0[:, x_edge_left:x_edge_right, y_edge_bottom:y_edge_top, :]
                p1 = HRRRv3_lead_p1[:, x_edge_left:x_edge_right, y_edge_bottom:y_edge_top, :]
                p2 = HRRRv3_lead_p2[:, x_edge_left:x_edge_right, y_edge_bottom:y_edge_top, :]
