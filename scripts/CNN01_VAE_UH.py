...
save_round = 2
seeds = 123456
model_prefix_load = 'VAE_UH{}'.format(weights_round) #False
model_prefix_save = 'VAE_UH{}'.format(save_round)
N_vars = L_vars = 15
lr = 1e-4
# ==================== #
...
with h5py.File(save_dir+'CNN_Validation_large.hdf', 'r') as h5io:
    VALID_input_64 = h5io['VALID_input_64'][..., 2:5]
...
OUT = keras.layers.Conv2D(3, kernel_size=1, padding='same', use_bias=True)(X)
...
model_final.fit(VALID_input_64, batch_size=64, epochs=100)
